...
f_mode = 6;% Which of the performance functions is to be used
...
num_experiments = 50;% Number of total trials
...
elseif f_mode == 2
    alpha = trig_amplitude*randn([num_traits,num_traits,num_frequencies])/num_frequencies^2; % second dimension is the upper frequency of sines used to construct the random performance function
...
elseif f_mode == 5
    f = @(x,y) Moran_performance_function_interp(x,y,'log_odds_moran_stag_10000_adjusted');
elseif f_mode == 6
    f = @(x,y) Moran_performance_function_interp(x,y,'log_odds_moran_chicken_10000_midway');
elseif f_mode == 7
    f = @(x,y) Moran_performance_function_interp(x,y,'log_odds_moran_stag_1000_midway');
end
