    ...
    while epoch < final_epoch && evolution_over == 0
        ...
        for i=0:(num_competitors/10-1)
            for j=1:10
                ...
                %end
                array_norm = max(abs(child_vector));
                if array_norm > 1
                    child_vector = 1/array_norm*child_vector;
                end
...
%% save structs
save('evolution_test_blotto_results.mat', 'results');

%% visualize data
PlotResults_Blotto('evolution_test_blotto_results.mat');
