...
for experiment = 1:num_experiments
    ...
    clf;
    hold on
    hist(competitor_traits,20,'fill')
    grid on
    axis square
    xlim([-0.1,1.1])
    ...
    while epoch < final_epoch && evolution_over == 0
        ...
        for l = 1:num_competitors
            for events = 1:games_per_competitor
                ...
                %% store competitors
                competitors(num_game,1) = l;
                competitors(num_game,2) = m;
                
                %% get win probability
                p_win(num_game) = (1 + exp(-competition(l,m)))^(-1); % p_win = logistic(performance)
                
            end
            
        end
        
        Outcomes = (Z <= p_win); % = 1 if l wins, 0 if m wins
        ...
        for k = 1:(round(num_competitors/10))%Getting an ill-conditioned covariance error
            GMModels{k} = fitgmdist(competitor_traits,k,'Options',options,'CovarianceType','full', 'RegularizationValue',genetic_drift/10);
        ...
        if n_classes == 1 && (norm(cluster_centroid - new_cluster_centroid)) < genetic_drift/5%Cluster centroid has stopped moving
            evolution_over = 1;
            final_epoch = epoch;
        end
...
%% save structs
save('evolution_test_4_results.mat', 'results');

%% visualize data
PlotResults_Moran('evolution_test_4_results.mat');
