function [rho,uncertainty] = estimate_rho_Gauss(f,centroid,var,tol,epoch_bounds,m)
...
while stop == 0
    ...
    %% sample competitors
    X = repmat(centroid,m,1) + sqrt(var)*randn(m,length(centroid));
    
    %% Calculate performance using the performance function
    xs = X(edge_to_endpoints(:,1),:);
    ys = X(edge_to_endpoints(:,2),:);
    competition = sparse(edge_to_endpoints(:,1), edge_to_endpoints(:,2),f(xs,ys),m,m) - ...
        sparse(edge_to_endpoints(:,2), edge_to_endpoints(:,1),f(xs,ys),m,m);
